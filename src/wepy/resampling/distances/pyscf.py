...
class ChargeDistance(Distance):
    ...
    def __init__(self, atom_indices=None, charge_key="charges"):
...
class ChargeTransferDistance(Distance):
    ...
    def __init__(self, donor, acceptor, charge_key="charges"):
...
class CremerPopleDistance(Distance):
    ...
    def __init__(self, ring_indices, coordinate="theta"):
        ...
        if self.N < 4 or self.N % 2 != 0:
            raise ValueError("ring must have an even number of atoms >= 4 (N=6 for cyclohexane)")
...
class HOMOLUMOGapDistance(Distance):
    ...
    def __init__(self, n_occ=None, units="hartree", mo_energy_key="mo_energy"):
    ...
    def image_distance(self, image_a, image_b):
        return float(abs(np.asarray(image_a, dtype=float)[0]
