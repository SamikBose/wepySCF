...
def _typecheck(session) -> None:
    session.run("mypy", "--strict", *TYPECHECK_TARGETS)


def qa_install(session: Session) -> None:
...
@nox.session
def black_check(session) -> None:
    qa_install(session)
    _black_check(session)


@nox.session
def isort_check(session) -> None:
    qa_install(session)
    _isort_check(session)
...
@nox.session
def interrogate(session) -> None:
    qa_install(session)
    _interrogate(session)


@nox.session
def docstring_lint(session) -> None:
    qa_install(session)
