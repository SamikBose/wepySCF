...
class PySCFRunner(Runner):
    ...
    def run_segment(self, walker, segment_length, platform=None, platform_kwargs=None, **kwargs):
        ...
        # Resolve platform: local arg > cycle-level > runner-level default
        if platform is None:
            platform = self._cycle_platform
        if platform is None:
        ...
        if platform_kwargs is None:
            raise ValueError("platform_kwargs must be provided with a 'DeviceIndex' key for GPU runs.")
        ...
        with cp.cuda.Device(gpu_idx):
            ...
            integrator = pyscf.md.integrators.LangevinMiddle(scanner, T=100.0, friction_coef=1.0, dt=self.delta_T)  # timestep in fs
            # integrator = pyscf.md.integrators.Langevin(scanner, T=100.0, friction_coef=1.0, dt=self.delta_T)  # timestep in fs
            new_integrator = integrator.run(veloc=updated_veloc, steps=segment_length)
            ...
            new_pyscf_state = PySCFState(new_mol, new_vel, new_accel, data=data)
...
KEYS = ("positions", "velocities", "acceleration", "elements", "kin", "pot", "temperature", "time")
class PySCFState(WalkerState):
    def __init__(self, mol_obj, vel, accel, data=None, **kwargs):
        self._data = {}
        self._data["positions"]    = mol_obj.atom_coords()
        self._data["velocities"]   = vel
        self._data["acceleration"] = accel
        self._data["elements"]     = mol_obj.elements

        if data is not None:
            self._data["kin"]         = data.get("kin")
            self._data["pot"]         = data.get("pot")
            self._data["temperature"] = data.get("temperature")
            self._data["time"]        = data.get("time")
        else:
            self._data["kin"]         = None
            self._data["pot"]         = None
